...
//  ----- Dados do Aluno -----
//  Nome: João Marcos De Souza
//  email:jmarcos.dev2002@gmail.com
//  Matrícula:20231160021
//  Semestre: 23.2
...
#include "JoaoMarcos-20231160021.h" // Substitua pelo seu arquivo de header renomeado
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
/*
...
DiasMesesAnos q2(char datainicial[], char datafinal[]) {
  ...
  if (q1(datainicial) == 0) {
    ...
    DataQuebrada data1, data2;
    data1 = quebraData(datainicial);
    data2 = quebraData(datafinal);
    int diastotais1 = data1.iDia + ((data1.iMes * 30) + (data1.iAno * 365));
    int diastotais2 = data2.iDia + ((data2.iMes * 30) + (data2.iAno * 365));
    ...
    int chegou = 0;
    int diaspassados = 0;
    int anoinicio = data1.iAno;
    int mesinicio = data1.iMes;
    int diainicio = data1.iDia;
    int diasNosMeses[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    while (chegou == 0) {

      int trocaAno = 0;
      if (ehBissexto(data1.iAno) == 1) {
        diasNosMeses[1] = 29;
      } else {
        diasNosMeses[1] = 28;
      }
      ...
      while (trocaAno == 0 && chegou == 0) {
        ...
        data1.iDia++;
        diaspassados++;
        if (data1.iDia > diasNosMeses[data1.iMes - 1]) {
          data1.iDia = 1;
          data1.iMes++;
        }

        if (data1.iDia == 31 && data1.iMes == 12) {
          trocaAno = 1;
        }

        if (data1.iDia == data2.iDia && data1.iMes == data2.iMes &&
            data1.iAno == data2.iAno) {
          chegou = 1;
        }
      }
...
int q5(int num) {
  int mult = 1, i = 0, base = 1, i2 = 0, r, resultado = 0;
...
int q6(int numerobase, int numerobusca) {
  ...
  while (numerobase > 0) {
    if (numerobusca <= i) {
      if (i > 1) {
        lastDigit = numerobase % i;
      } else {
        lastDigit = numerobase % 10;
      }

      if (lastDigit == numerobusca) {
        qtdOcorrencias++;
        if (numerobusca > 9) {
